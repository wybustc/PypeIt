...
class P200NGPSSpectrograph_u(P200NGPSSpectrograph):
    """
    Child to handle P200/NGPS u-Channel specific code (added by YB)
    """
    name = 'p200_ngps_u'
    camera = 'NGPS_u'
    header_name = 'NGPS_u'
    ...
    def compound_meta(self, headarr: List[fits.Header], meta_key: str):
        ...
        if meta_key == 'mjd':
            ...
            # Always the same binning for DET01 and DET02    
            binspat = headarr[3]['BINSPAT'] 
            binspec = headarr[3]['BINSPEC']
            return parse.binning2string(binspec, binspat)
    ...
    def get_detector_par(self, det: int, hdu: Optional[fits.HDUList] = None):
        ...
        if hdu is None:
            ...
            binning = self.get_meta_value(self.get_headarr(hdu), 'binning')
            datasec = np.atleast_1d(parse.flip_fits_slice(hdu[3].header['DATASEC']))
            oscansec = np.atleast_1d(parse.flip_fits_slice(hdu[3].header['BIASSEC']))

        # Detector 1 (r Channel)
        detector_dict1 = dict(
            binning         = binning,
            det             = 1, # All r channel images assigned to extension 1
            dataext         = 3, # All r channel images assigned to extension 1
            specaxis        = 2,
            specflip        = False, 
            spatflip        = False, 
            platescale      = 0.5,    #NOTE by (YB), the data below are taken from r-channel (i-channel also is same?)
            darkcurr        = 0.0,  # e-/pixel/hour (No dark current)
            saturation      = 65000., # ???
            nonlinear       = 40./45.,
            mincounts       = -1e10, # check
    ...
    def default_pypeit_par(cls):
        ...
        par['calibrations']['slitedges']['sync_predict'] = 'nearest'
        par['calibrations']['slitedges']['edge_thresh'] = 20. # Lower edge tracing thresdhold to catch leftmost slit
        par['calibrations']['slitedges']['minimum_slit_length'] = 100 # Set minimum slit length 
        par['calibrations']['slitedges']['min_edge_side_sep'] = 1.0
#        #par['calibrations']['slitedges']['add_slits'] = ['1:1090:24:153']
        ...
        par['calibrations']['wavelengths']['lamps'] = ['FeI','ArI','ArII'] # FeAr and ThAR lamps for NGPS (ThAr)
        par['calibrations']['wavelengths']['method'] = 'holy-grail' # Use wavelength template
        par['calibrations']['wavelengths']['sigdetect'] =1.0
        # par['calibrations']['wavelengths']['method'] = 'full_template'
        # par['calibrations']['wavelengths']['reid_arxiv'] = 'p200_dbsp_blue_600_4000_d55.fits' #  Channel
